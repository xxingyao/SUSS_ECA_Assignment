...
@dashboard.route('/user_bar_chart', methods=['GET', 'POST'])
def user_bar_chart():
    if request.method == 'GET':
        return render_template('user_bar_chart.html', panel="User Chart")
    elif request.method == 'POST':
        user_booking_counts = Booking.getBookingsGroupedByUserAndStatus()
        sorted_user_booking_counts = dict(sorted(user_booking_counts.items()))


        chart_data = {
            'labels': list(sorted_user_booking_counts.keys()),
            'cancelled': [counts['cancelled'] for counts in sorted_user_booking_counts.values()],
            'completed': [counts['completed'] for counts in sorted_user_booking_counts.values()],
            'upcoming': [counts['upcoming'] for counts in sorted_user_booking_counts.values()]
        }
